...
//   const { receiverId, receiverName, receiverImage, type } = route.params;
...
//   const channelName =
//     type === 'support' ? receiverId : [senderId, receiverId].sort().join('');

//     console.log("LINE AT 50", channelName);
...
//       console.log('LINE AT 96', data?.data?.data);

//       if (data?.data?.data && data?.data?.data.length > 0 && data?.data?.data[0].messages) {
...
//   console.log('LINE AT 334', messages);
...
const ChatScreen = () => {
  ...
  // const renderMessage = (msg, index) => {
  //   const isMine = msg.senderId === senderId;
  //   console.log('LINE AT 865', msg);

  //   // Render image message
  //   if (msg.isImage && msg.files && msg.files.length > 0) {
  //     return (
  //       <View
  //         key={msg.id || index}
  ...
  //           <View
  //             className={`bg-gray-200 rounded-2xl px-3 py-2 ${
  //               isMine ? 'bg-blue-500' : ''
  //             }`}
  //           >
  //             <ScrollView horizontal showsHorizontalScrollIndicator={false}>
  //               {msg?.files.length > 0 &&
  //                 msg?.files.map((imageUrl, idx) => (
  //                   <Image
  //                     key={idx}
  //                     source={{ uri: imageUrl }}
